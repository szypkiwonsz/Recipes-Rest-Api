...
class RecipeSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        """
        Method for writable nested serializer
        """
    ...
    def update(self, instance, validated_data):
        ...
        for ingredient_data in ingredients_data:
            ...
            foods_data = [ingredient_data.pop('food', [])]
            for food_data in foods_data:
                food_qs = Food.objects.filter(name__iexact=food_data['name'])
                if food_qs.exists():
                    food = food_qs.first()
                else:
                    food = Food.objects.create(**food_data)

                ingredient_qs = Ingredient.objects.filter(unit__iexact=ingredient_data['unit']).filter(
